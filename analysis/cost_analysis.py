...
class CostEstimator:
    ...
    def calculate_battery_cost(self, result_dir):
        ...
        for root, dirs, files, in os.walk(".", topdown=True):
            for file in files:
                ...
                if 'battery' in path_lst:
                    battery_LOL = 1 - pd.read_csv(file)['SOH'].to_numpy()[-1]
                    capital_loss_to_aging = (battery_LOL/0.2 * capital_cost)/scenario['pack_energy_cap'] # assuming 0.8 is end of life
    ...
    def calculate_electricity_cost_PGEBEV2s(self, result_dir, PGE_seperate_file=True):
        ...
        for root, dirs, files, in os.walk(".", topdown=True):
            for file in files:
                path_lst = file.split("_")
                if 'station' in path_lst and 'block' not in path_lst:
