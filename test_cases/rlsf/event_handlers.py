...
def on_precommit(t):
    ...
    if gblvar.it>0:
        ...
        gblvar.Q=(1/gblvar.lam)*(gblvar.Q-(np.outer(np.matmul(gblvar.Q,gblvar.x_array_aug[gblvar.it-1,:].reshape(-1,1)),np.matmul(gblvar.Q,gblvar.x_array_aug[gblvar.it-1,:].reshape(-1,1))))/(gblvar.lam+np.dot(gblvar.x_array_aug[gblvar.it-1,:],np.matmul(gblvar.Q,gblvar.x_array_aug[gblvar.it-1,:].reshape(-1,1)))))
