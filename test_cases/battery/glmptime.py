    ...
    with open(tz_file) as f:
        ...
        for line in f:
            ...
            if match:
                ...
                if "." in match[3]:
                    hour_offset = match[3].split('.')[0]
                    min_offset = str(int(6 * int(match[3].split(".")[1])))
                    if float(min_offset) < 10:
                        min_offset = f"0{min_offset}"
                else:
                    hour_offset = match[3]
                    min_offset = "00"
                if int(hour_offset) < 10:
                    hour_offset = f"0{hour_offset}"
                if int(hour_offset) == 0:
                    hour_offset = "00"
                if match[2] is None:
                    tz_offset.append(f"+{hour_offset}{min_offset}")

                else:  # no sign in timestamp, assume +
                    tz_offset.append(match[2] + hour_offset + min_offset)
        ...
        for i, item in enumerate(t_tz_list_ds):
            if t_tz_list_ds[i] is not None:  # Discard all the empty DS values
                offset_adj = (int(tz_offset[i]) - 100) * (-1)
                TZ_dict[t_tz_list_ds[i]] = format(offset_adj, '05') if offset_adj < 0 else "+" + format(offset_adj,
                                                                                                        '04')
