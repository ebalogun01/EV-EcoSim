...
def sim_transformer():
    '''
    Propage transformer state from previous timestep to current timestep (currently it is a minute resolution data
        ...
        if gblvar.trans_int_method == 'euler':
            ...
            for i in range(len(gblvar.trans_list)):
                ...
                for j in range(int(gblvar.pf_dt/gblvar.trans_dt)):
                    trans_To_new[i]=trans_To_new[i]+gblvar.trans_dt*(((gblvar.trans_R*(gblvar.trans_power[i]/gblvar.trans_rated_s[i])**2+1)/(gblvar.trans_R+1))
                    ...
                    trans_Th_new[i]=trans_Th_new[i]+gblvar.trans_dt*(((gblvar.trans_power[i]/gblvar.trans_rated_s[i])**2)*
                                                                     ((gblvar.trans_delta_theta_hs_rated**(1/gblvar.trans_m))/(gblvar.trans_tau_h))
