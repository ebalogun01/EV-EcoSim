...
# file: Cyme2DSS.py
# src: https://github.com/GRIDAPPSD/CIMHub/tree/master/converters
...
def CreateRecloser(f, Name, row):
    ...
    if row[3] == 'Open':
        f.write(' action=open\n')
        f.write(
            'edit line.' + Name + ' enabled=no // TODO: fix OpenDSS so that action=open is executed for the initial powerflow')
    f.write('\n')
    return
...
def CreateTransformer(f, Name, row, cfg):
    ...
    if row[6] == 'bypassed':
        f.write(
            'new line.Transformer_' + Name + ' phases=' + str(nphs) + ' bus1=' + row[0] + phs + ' bus2=' + row[1] + phs)
...
def CreateRegulator(f, Name, row):
    ...
    if nMon < nPhs:  # ganged
        ...
        kva3 = 3 * row[5]
        f.write('new transformer.{:s} phases={:d} windings=2 buses=({:s},{:s})'.format(Name, nPhs, row[0] + allPhs,
                                                                                       row[1] + allPhs))
        f.write(
            ' kvas=({:.2f},{:.2f}) kvs=({:.3f},{:.3f}) conns=({:s},{:s}) xhl=1 enabled={:s}\n'.format(kva3, kva3, kvLL,
                                                                                                      kvLL, row[4],
                                                                                                      row[4], row[11]))
    ...
    for Section in root.findall(xstr):
        ...
        for Devices in Section.findall('Devices'):
            ...
            for child in Devices:
                ...
                if DeviceType in ('Switch', 'Breaker', 'Recloser') and (
                        SectionLine == 'Yes'):  # not inserting node for 'Fuse'
            ...
            for child in Devices:
                ...
                if DeviceType == 'OverheadLine':
                    ...
                    if DeviceLength > 0.0:
                        ...
                        if (row[0] > row[1]) and (WireN == 'OH_NONE'):
                            WireN = WireA
                        OHPhaseTable[DeviceNumber] = [LineFromNode, LineToNode, Phase, DeviceLength, LineSpacing, WireA,
                                                      WireB, WireC, WireN]
                    else:
                        # add a switch matching SectionStatus
                        SwitchTable[DeviceNumber] = [LineFromNode, LineToNode, Phase, SectionStatus, DeviceType]
                ...
                #                        print ('Fuse', FuseTable[DeviceNumber])
                    ...
                    enabled = Connected(child)
                    TransformerPhaseTable[DeviceNumber] = [XfmrFromNode, XfmrToNode, Phase, XfmrCode1, XfmrCode2,
                                                           XfmrCode3, enabled]
                    ...
                    if child.find('ReverseSensingMode').text != 'NoReverse':
                        print('WARNING: unhandled ReverseSensingMode for regulator ' + DeviceNumber + ': ' + child.find(
                            'ReverseSensingMode').text)
                    if child.find('SettingOption').text != 'LoadCenter':
                        print('WARNING: unhandled SettingOption for regulator ' + DeviceNumber + ': ' + child.find(
                            'SettingOption').text)
                    RegulatorTable[DeviceNumber] = [XfmrFromNode, XfmrToNode, Phase, monPhs, conn, kva, kv, vreg, bw,
                                                    pt, ct, enabled]
...
// This file is not over-written if you run the converter again.""", file=sp)
        if len(SourceTable) < 1:
            ...
            print('new circuit.{:s} bus1={:s} basekv={:.3f} pu=1 ang=0 r1=0 x1=0.001 r0=0 x0=0.001'.format(RootName,
                                                                                                           '???',
                                                                                                           DefaultBaseVoltage),
                  file=sp)
