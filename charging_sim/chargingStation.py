...
class ChargingStation:
    ...
    """
    def __init__(self, config, controller=None, storage=None, solar=None, num_evs=1600, status='idle'):
        self.config = config
        self.id = self.config["locator_index"]
        self.loc = config["location"]
        self.power_factor = config["power_factor"]
        self.load_path = config["data_path"]
        ...
        # Run some methods for setup.
        self.load_charging_data()   # Load the charging data

    def load_charging_data(self, path=None):
        """
        Loads its own unique charging data to be used during simulation. In prior versions, this was done in
        orchestrator.py. However now, new individual charging stations will load their own data.

        :return: None.
        """
        if path:
            self.load_data = np.loadtxt(path).flatten()
        else:
            self.load_data = np.loadtxt(self.load_path).flatten()
            assert len(self.load_data) >= num_steps, "Load data is not the same length as the number of steps!"
...
class ChargingStationCentralized:
    """
    Class for charging stations with centralized Distributed Energy Resources (solar and storage).
    ...
    :param str status: Charging station status. Default 'idle'.
    """
    def __init__(self, storage, config, controller, num_evs=1600, solar=None, status='idle'):
        self.config = config
        self.id = self.config["locator_index"]
        self.loc = config["location"]
        self.power_factor = config["power_factor"]
    ...
    def save_sim_data(self, save_prefix: str):
        ...
        pd.DataFrame(data).to_csv(f'{save_prefix}/charging_station_sim_{save_file_base}.csv')
        # Save JSON
        with open(f'{save_prefix}/charging_station_config_{self.id}_{self.loc}.json', "w") as outfile:
            json.dump(self.config, outfile, indent=1)
        print('***** Successfully saved simulation outputs to: ', f'charging_station_sim_{save_file_base}.csv')
