...
def plot_results(test_demand, battery, solar):
    ...
    ax1.plot_tables(test_demand)
    after_storage_demand = test_demand + battery.topology[2]*battery.true_power - solar
    energy_only_solar = test_demand - solar
    ax1.plot_tables(after_storage_demand, marker='o', linewidth=2, linestyle='dashed')
    ax1.plot_tables(energy_only_solar)
    ...
    ax2.plot_tables(battery.SOC.value, color='r')
